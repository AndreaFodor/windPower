...
- `arima_model/`: Contains exploratory data analysis, initial trials of some time series models, and the main ARIMAx model.
    ...
    - `models/`: Contains Python script for the ARIMAx model and a few baseline models:
        ...
        - `timeseries.py`: The main Python script for the **ARIMAx** model.
- `knn_model/`: Contains all relevant folders for the **kNN Regressor** model - includes its own *README*.
...
- `random_forests/`: Contains our attempts to solve the problem using Random Forest and XGBoost models - includes its own *README*.
