...
class ARIMAxPredictions():
    ...
    def forecast(self, date):
        ...
        df_train['y'] = df_train['y'].shift(periods=-1)
        stfcst = StatsForecast(models=self.models, 
                           freq='D', 
                           n_jobs=-1,
                           )
        date = date - pd.Timedelta(1,unit='d')
        fcst = stfcst.forecast(df=df_train[(df_train['ds']<date) & (df_train['ds']>(date - pd.Timedelta(days=self.training_interval + 1)))], 
                           h=self.horizon, 
                           X_df=df_train.drop(columns='y')[df_train['ds']==date],
    ...
    def forecast_range(self, start_date, end_date, plotting=True):
        ...
        cmape = mape(self.df_train[(self.df_train['ds']>=start_date) & (self.df_train['ds']<=end_date)].y.values, 
                     self.forecastarr)
